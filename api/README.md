...
15. **Protocol Coffee and Tea** (`server/api/coffee-and-tea.js`) — a full
    ...
    **Scope notes / what's deliberately NOT built:**
    ...
    - **PDF export uses the actual styled render layer from the other
      chat** (`api/render-coffee-and-tea-pdf.py`, `render_pdf_lib.py`,
      `schema.py`) — a Python/reportlab renderer producing the proper
      designed layout (green color scheme, Appendix A/B/C/D structure,
      styled tables) rather than the plain jsPDF approximation originally
      built here before that file existed. The frontend now calls this
      endpoint and downloads the returned PDF directly, instead of
      building the PDF client-side. Validates the reasoning layer's output
      through the real pydantic `ReasoningOutput` schema before rendering
      - a second, stricter validation layer on top of the JS checks
      already in `coffee-and-tea.js`.
